...
import time
import keyboard
class Game(tf_agents.environments.py_environment.PyEnvironment):
    def __init__(self, discount = 1.):
        super().__init__()
        self._action_spec = tf_agents.specs.BoundedArraySpec(
            shape=(), dtype=np.int32, name="action", minimum=0, maximum=3)
        self._observation_spec = tf_agents.specs.BoundedArraySpec(
            shape=(4, 4), dtype=np.int32, name="observation", minimum=0, maximum=2048)
    ...
    def _reset(self):
        ...
        return tf_agents.trajectories.time_step.restart(self.board)
    ...
    def _step(self, action):
        ...
        if done:
            if self.result() == 1: reward += 1000
            #print(self.result())
            #print("terminating")
            return tf_agents.trajectories.time_step.termination(self.board, reward)
        else:
            return tf_agents.trajectories.time_step.transition(self.board, reward, self.discount)
